...
def update_texture_metadata():
    ...
    for category_dir in textures_dir.iterdir():
        ...
        # Initialize category in metadata
        ...
        for ext in ('*.jpg', '*.jpeg', '*.png'):
            texture_files.extend(category_dir.glob(ext))
        ...
        for filepath in texture_files:
            ...
            base_metadata = extract_metadata_from_filename(relative_path)
            
            # Get texture-specific description (title already set from filename)
            texture_display = get_texture_metadata(category_name, base_metadata['filename'])
            base_metadata.update({
                "description": texture_display["description"]
            })
            
            metadata["textures"][category_key]["textures"].append(base_metadata)
    ...
    with open('texture_metadata.json', 'w', encoding='utf-8') as f:
